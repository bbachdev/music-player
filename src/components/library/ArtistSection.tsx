      ...
      <div className={`flex flex-col h-full w-full`}>
        ...
        <ScrollArea className={`w-full`}>
          <div className={`flex flex-col text-left`}>
              ...
              return <button key={artist.name} className={`dark:hover:bg-slate-700/90 text-left p-2 ${selectedArtist === artist.id ? 'bg-slate-700/90' : ''}`} onClick={() => setSelectedArtist(artist.id)}><span className={`p-1`}>{artist.name}</span></button>
            })}
          </div>
